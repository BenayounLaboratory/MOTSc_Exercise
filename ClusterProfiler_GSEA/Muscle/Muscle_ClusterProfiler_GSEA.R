# This script perform GSEA at FDR 15 using the clusterProfiler package for Muscle DESeq2 all gene statistics
...
res.muscle <- read.csv("2019-03-27_Muscle_MOTSc_global__all_genes_statistics_sva.txt", sep = "\t", header = T)

entrezID.background  <- bitr(rownames(res.muscle), 
                            fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Mm.eg.db")
# 14884 mapping correcly to annotation for Muscle

######################################################

# prepare GeneLists using DEseq2 t-statistic to rank genes
res.muscle <- as.data.frame(res.muscle)
res.muscle$symbol <- rownames(res.muscle)
res.muscle <- merge(res.muscle, entrezID.background, by.x = "symbol", by.y = "SYMBOL")
Muscle.geneList = res.muscle$stat
names(Muscle.geneList) = res.muscle$ENTREZID
Muscle.geneList = sort(Muscle.geneList, decreasing = TRUE)

## GO Gene Set Enrichment Analysis
go.bp.gsea <- gseGO(geneList     = Muscle.geneList,
                    OrgDb        = org.Mm.eg.db,
                    keyType      = "ENTREZID",
                    ont          = "BP",
                    nPerm        = 1000,
...
write.table(go.bp.gsea@result, file = paste(Sys.Date(),"Muscle_GOBP_GSEA_Analysis_FDR15.txt", sep = "_"), quote = F, sep = "\t")
head(go.bp.gsea)

# GSEA plot of interesting pathways
pdf(paste(Sys.Date(),"Muscle_GO_BP_FDR15_Selected_GSEA_plots.pdf", sep = "_"))
...
kegg.gsea <- gseKEGG(geneList     = Muscle.geneList,
                     organism     = 'mmu',
                     keyType      = 'ncbi-geneid',
                     nPerm        = 1000,
                     pvalueCutoff = 0.15,
...
# write results to file
write.table(kegg.gsea@result, file = paste(Sys.Date(),"Muscle_KEGG_GSEA_Analysis_FDR15.txt", sep = "_"), quote = F, sep = "\t")
head(kegg.gsea)

# GSEA plot of interesting pathways
pdf(paste(Sys.Date(),"Muscle_KEGG_FDR15_Selected_GSEA_plots.pdf", sep = "_"))
