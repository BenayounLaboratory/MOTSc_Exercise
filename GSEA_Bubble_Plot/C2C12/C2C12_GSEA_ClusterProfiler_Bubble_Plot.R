...
# source code
get_enrich_balloons("GO-BP_FDR15_pathways", 15, "GO-BP")
get_enrich_balloons("KEGG_FDR15_pathways", 15, "KEGG")

get_enrich_balloons("GO-BP_FDR5_pathways", 5, "GO-BP")
# KEGG FDR 5% had no significant pathways, therefore no bubble plot can be generated

####################
##### FUNCTION #####
####################
# This function makes bubble plots from C2C12 GSEA statistics (FDR 5 and 15)
# INPUT: my.data.name = output file name describing the gene set name and FDR
#        FDR = false discovery rate (0.05, 0.15)
...
get_enrich_balloons <- function(my.data.name, FDR, gene.set, my.threshold = 0,
  ...
  my.txtname <- paste(Sys.Date(),"C2C12_Enrichment_table",my.data.name,"pathways_significant_in",my.threshold,"or_more.txt", sep="_")
  ...
  my.pdfname <- paste(Sys.Date(),"C2C12_Enrichment_BALLOON_plot",my.data.name,"pathways_significant_in",my.threshold,"or_more.pdf", sep="_")
  
  if (gene.set == "GO-BP" & FDR == 5) {
    pdf(my.pdfname, onefile=T, height = 25, width=15)
  }
  
  if (gene.set == "GO-BP" & FDR == 15) {
    pdf(my.pdfname, onefile=T, height = 100, width=15)
  }

  if (gene.set == "KEGG" & FDR == 15) {
    pdf(my.pdfname, onefile=T, height = 25, width=15)
  }
